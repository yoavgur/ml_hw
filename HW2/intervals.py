from numpy import array, concatenate, cumsum, zeros, argmin, arange
    ...
    for i in range(1, m+1):
        for j in range(1, k+1):
            ...
            for k in range(0,i+1):  
                next_errors = E[k,j-1] + (cy[i]-cy[k]) + concatenate([[0], cumsum((-1)**(ys[arange(k, i)] == 1))])
                min_error = argmin(next_errors)
                options.append((next_errors[min_error], (k, arange(k,i+1)[min_error])))
